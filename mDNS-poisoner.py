import time, os, sys, struct, socket #struct to convert python vars into byte-strings
from socketserver import UDPServer, ThreadingMixIn
from socketserver import BaseRequestHandler
from threading import Thread
from dnslib import *

MADDR = ('224.0.0.251',5353) #mDNS multicast socket
class UDP_server(ThreadingMixIn,UDPServer):
    allow_reuse_address = True
    def server_bind(self):
        self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR,1)
        mreq = struct.pack("=4sl",socket.inet_aton(MADDR[0]),socket.INADDR_ANY) #struct.pack converts to
